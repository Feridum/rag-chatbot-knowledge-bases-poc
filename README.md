...
- 🔄 **Real-time Context** - Automatically enhances queries with relevant document context
...
│   ├── api/
│   │   └── chat/
│   │       └── route.ts          # Chat API with RAG integration
...
├── documents/                    # Local files to upload for ingestion
├── scripts/
│   ├── setup-knowledge-base.ts   # AWS infrastructure + upload + ingest
│   └── upload-documents.ts       # Upload local docs + start ingestion
├── lib/
